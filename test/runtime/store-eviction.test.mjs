...
test("a failed completion rollover keeps the entry until the run is truly terminal (fx3)", async () => {
  ...
  const failed = await runtime.transition({ key: "root/frame", status: "completed", met: ["framed"], checkpoint: cp("framed") }, undefined, h.ctx);
  ...
  const retried = await runtime.transition({ key: "root/frame", status: "completed", met: ["framed"], checkpoint: cp("framed") }, undefined, h.ctx);
